...
class BerkJones(ScoringFunction):
    ...
    def __init__(self, **kwargs):
        ...
        'alpha (float)' - the alpha threshold that will be used to compute the score.
            In practice, it may be useful to search over a grid of alpha thresholds
            and select the one with the maximum score.


        [1] Neill, D. B., & Lingwall, J. (2007).
        A nonparametric scan statistic for multivariate disease surveillance.
        Advances in Disease Surveillance, 4(106), 570
        """
    ...
    def score(
        ...
        assert (
            q > 0
        ), f"""Warning: calling compute_score_given_q with
            observed_sum={observed_sum:.2f},
            expectations of length={len(expectations)},
            penalty={penalty:.2f},
            q={q:.2f}"""
        ...
        part_a = observed_sum * np.log(q / alpha)
        part_b = (len(expectations) - observed_sum) * np.log((1 - q) / (1 - alpha))
        ans = part_a + part_b - penalty
